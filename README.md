...
このリポジトリ **ai-ci-auto-fixer** を初めて開いた人は、まずここだけ見れば初期設定から実行、成果物確認まで進められます。
...
- Variable: `AI_MODEL=gpt-5.1` など

Claude を使う場合:

- Secret: `ANTHROPIC_API_KEY`
- Variable: `AI_PROVIDER=anthropic`
- Variable: `AI_MODEL=claude-sonnet-4-5` など

Codex CLI / Claude Code / 任意コマンドを使う場合:

- Secret: `AI_FIX_COMMAND`
- Variable: `AI_PROVIDER=command`

通常は `AI_FIXER_GITHUB_TOKEN` は不要です。各 repo の `GITHUB_TOKEN` を使って同じ repo に branch push と PR 作成をします。

ただし、Organization 全体の private repo を横断したい、権限を明示管理したい、GitHub App token を使いたい場合は Secret `AI_FIXER_GITHUB_TOKEN` を設定してください。
